...
set "ACTION=\"%PYTHON_EXE%\" \"%APP_FILE%\" --offline --offline-model small --tray"

schtasks /Create /TN "%TASK_TRAY_OFFLINE%" /SC ONLOGON /TR "%ACTION%" /RL LIMITED /F >nul
if errorlevel 1 (
  echo Task Scheduler nicht verfuegbar. Nutze Startup-Ordner-Fallback...
  > "%STARTUP_TRAY_OFFLINE%" echo @echo off
  >> "%STARTUP_TRAY_OFFLINE%" echo start "Blitztext Tray Offline" /min "%PYTHON_EXE%" "%APP_FILE%" --offline --offline-model small --tray
  if not exist "%STARTUP_TRAY_OFFLINE%" (
    echo Konnte weder Task noch Startup-Fallback einrichten.
    pause
    exit /b 1
  )
  echo Autostart aktiviert: TRAY OFFLINE ^(Startup-Ordner Fallback^)
  echo Datei: %STARTUP_TRAY_OFFLINE%
  echo.
...
echo Autostart aktiviert: TRAY OFFLINE
echo Task: %TASK_TRAY_OFFLINE%
echo.
echo Hinweis: Beim naechsten Login startet Blitztext direkt im Tray.
pause
